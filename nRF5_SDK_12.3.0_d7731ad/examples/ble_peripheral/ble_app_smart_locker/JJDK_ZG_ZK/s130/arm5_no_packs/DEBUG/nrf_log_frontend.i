...
#line 47 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_frontend.c"
#line 48 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_frontend.c"
#line 49 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_frontend.c"


;enum { static_assert_51 = 1 / (! !(((256 == 0) || ( ((256) != 0) && ((((256) - 1) & (256)) == 0) )))) };
...
typedef struct
{
    ...
    uint32_t                  buffer[256];
    nrf_log_timestamp_func_t  timestamp_func;  
    nrf_log_std_handler_t     std_handler;     
    nrf_log_hexdump_handler_t hexdump_handler; 
} log_data_t;

static log_data_t   m_log_data;

static const char * m_overflow_info = "\x1B[1;31m" "Overflow\r\n";
...
#line 213 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_frontend.c"

#line 238 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_frontend.c"

#line 258 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_frontend.c"
...
ret_code_t nrf_log_init(nrf_log_timestamp_func_t timestamp_func)
{
    ...
    ret_code_t err_code = nrf_log_backend_init(1 ? 0 : 1);
...
static inline _Bool buf_prealloc(uint32_t nargs, uint32_t * p_wr_idx)
{
    nargs += ((0) ? 2 : 1);
    uint32_t ovflw_tag_size = ((0) ? 2 : 1);
    _Bool     ret            = 1;
    ...
    *p_wr_idx = m_log_data.wr_idx;
    ...
    uint32_t required_words  = nargs + ovflw_tag_size; 
    if (required_words > available_words)
    {
        if (available_words >= ((0) ? 2 : 1))
        {
            
            nrf_log_header_t header = { . std = { . type = 1U, . raw = ((5U) & (1U << 4U)), . severity = (5U) & 0x07, . nargs = (0), . addr = ((uint32_t)(m_overflow_info) & ((uint32_t)(1U << 22) - 1U)) } };
...
#line 429 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_frontend.c"
...
uint32_t nrf_log_push(char * const p_str)
{
    ...
    uint32_t buflen    = (((slen) + (4) - 1) / (4));
    uint32_t offset    = 0;
    uint32_t wr_idx;
    char   * p_dst_str = (char *)cont_buf_prealloc(buflen, &offset, &wr_idx);
    if (p_dst_str)
    {
        nrf_log_header_t header = { . pushed = { . type = 0U, . offset = (offset), . len = (buflen), } };
...
void nrf_log_frontend_std_0(uint8_t severity, char const * const p_str)
{
    ...
    if (buf_prealloc(nargs, &wr_idx))
    {
        ...
        nrf_log_header_t header = { . std = { . type = 1U, . raw = ((severity) & (1U << 4U)), . severity = (severity) & 0x07, . nargs = (nargs), . addr = ((uint32_t)(p_str) & ((uint32_t)(1U << 22) - 1U)) } };
        m_log_data.buffer[wr_idx++ & mask] = header.raw;
...
void nrf_log_frontend_hexdump(uint8_t            severity,
                              ...
                              uint16_t           length)
{
#line 682 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_frontend.c"
    uint32_t mask   = m_log_data.mask;

    uint32_t wr_idx;
    if (buf_prealloc((((length) + (4) - 1) / (4)) + 1, &wr_idx))
    {
        nrf_log_header_t header = { . hexdump = { . type = 2U, . raw = ((severity) & (1U << 4U)), . severity = (severity) & 0x07, . offset = 0, . len = length, } };
        m_log_data.buffer[wr_idx++ & mask] = header.raw;
...
_Bool nrf_log_frontend_dequeue(void)
{
    ...
    uint32_t * p_timestamp = 0 ?
                             &m_log_data.buffer[rd_idx++ & mask] : 0;
