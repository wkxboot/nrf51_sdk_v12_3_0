...
#line 41 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_backend_serial.c"
...
ret_code_t nrf_log_backend_init(_Bool blocking);
...
#line 56 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_backend_serial.c"



static char m_uart_buffer[256];
static nrf_drv_uart_t m_uart = { . reg = {(void *)((NRF_UART_Type *) 0x40002000UL)}, . drv_inst_idx = 0,};
...
static _Bool m_blocking_mode = 0;
static const char m_default_color[] = "\x1B[0m";


static volatile _Bool m_rx_done = 0;
...
ret_code_t nrf_log_backend_init(_Bool blocking)
{
    ...
    uart_config.baudrate =
        (nrf_uart_baudrate_t)30924800;
    nrf_drv_uart_uninit(&m_uart);
    ret_code = nrf_drv_uart_init(&m_uart, &uart_config,
                                 blocking ? 0 : uart_event_handler);
    if (ret_code != ((0x0) + 0))
    {
        return ret_code;
    }
...
static _Bool serial_is_busy(void)
{
    _Bool res = 0;
...
static _Bool serial_tx(uint8_t * p_buf, uint32_t len)
{
    _Bool ret = 1;
...
#line 187 "..\\..\\..\\..\\..\\..\\components\\libraries\\log\\src\\nrf_log_backend_serial.c"
    return ret;
}
...
static _Bool buf_len_update(uint32_t * p_buf_len, int32_t new_len)
{
    _Bool ret;
    if (new_len < 0)
    {
...
static _Bool timestamp_process(const uint32_t * const p_timestamp, char * p_str, uint32_t * p_len)
{
    int32_t len = 0;
    _Bool    ret = 1;
    if (p_timestamp)
    {
        ...
        *p_len += len;
        ...
        len = snprintf(&p_str[len],256, "[%0" "8" "d]", (int)*p_timestamp);
...
static _Bool nrf_log_backend_serial_std_handler(
    uint8_t                severity_level,
    const uint32_t * const p_timestamp,
    const char * const     p_str,
    uint32_t             * p_args,
    uint32_t               nargs)
{
    ...
    switch (nargs)
    {
        ...
        case 1:
            tmp_str_len = snprintf(&str[buffer_len], 256-buffer_len, p_str, p_args[0]);

            break;

        case 2:
            tmp_str_len = snprintf(&str[buffer_len], 256-buffer_len, p_str, p_args[0], p_args[1]);
            break;

        case 3:
            tmp_str_len = snprintf(&str[buffer_len], 256-buffer_len, p_str, p_args[0], p_args[1], p_args[2]);
            break;

        case 4:
            tmp_str_len =
                snprintf(&str[buffer_len], 256-buffer_len, p_str, p_args[0], p_args[1], p_args[2], p_args[3]);
            break;
    ...
    uint32_t full_buff_len = 1 ?
            buffer_len + sizeof(m_default_color)-1 : buffer_len;
    if (status && (full_buff_len <= 256))
    {
        if (1)
        {
    ...
    uint32_t               buf1_length)
{
    ...
    if (offset == 0)
    {
        ...
        slen = (slen > (256 - (3*16 + 8 + 4 + 2))) ? (256 - (3*16 + 8 + 4 + 2)) : slen;
    ...
    do
    {
        ...
        for (byte_in_line = 0; byte_in_line < 16; byte_in_line++)
        {
            if (byte_cnt >= length)
            {
            ...
            else
            {
                ...
                p_hex_part    += 2; 
                *p_hex_part++  = ' ';
                *p_char_part++ = ((* __rt_ctype_table())[c] & (8+16+32+2+4)) ? c : '.';
                byte_cnt++;
            }
        }
        *p_char_part++ = '\r';
